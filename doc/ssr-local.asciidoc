...
NAME
----
ssr-local - shadowsocks client as socks5 proxy, libev port
...
*Shadowsocks-libev* consists of five components. `ssr-local`(1) works as a standard
...
EXAMPLE
-------
`ssr-local`(1) can be started from command line and run in foreground.
Here is an example:
....
# Start ssr-local with given parameters
ssr-local -s example.com -p 12345 -l 1080 -k foobar -m aes-256-cfb
....
