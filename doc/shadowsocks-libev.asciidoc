...
SYNOPSIS
--------
*ssr-local*|*ssr-redir*|*ssr-server*|*ssr-tunnel*|*ssr-manager*
...
*Shadowsocks-libev* consists of five components. One is `ssr-server`(1)
that runs on a remote server to provide secured tunnel service.
`ssr-local`(1) and `ssr-redir`(1) are clients on your local machines to proxy
traffic(TCP/UDP or both).
`ssr-tunnel`(1) is a tool for local port forwarding.

While `ssr-local`(1) works as a standard socks5 proxy, `ssr-redir`(1) works
as a transparent proxy and requires netfilter's NAT module. For more
information, check out the 'EXAMPLE' section.

`ssr-manager`(1) is a controller for multi-user management and traffic
statistics, using UNIX domain socket to talk with `ssr-server`(1).
