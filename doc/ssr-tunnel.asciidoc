...
NAME
----
ssr-tunnel - shadowsocks tools for local port forwarding, libev port
...
See 'OPTIONS' section for special option needed by `ssr-tunnel`(1).
...
Meanwhile, you configure `ssr-tunnel` to listen on 0.0.0.0:8388 and bind to eth1.
